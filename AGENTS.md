...
- This repository currently contains a minimal Rust Cargo workspace and a `cyclops` crate with CLI parsing, error types, event schema serialization types, event clock support, bounded async JSONL event emission, worktree-rooted path safety helpers, and OpenAI-shape chat message serialization types.
