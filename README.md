...
Cyclops is an early Rust coding-agent harness. The repository has a minimal Cargo workspace, `cyclops` CLI parser, typed event schema, event clock support, an async JSONL event emitter, worktree-rooted path safety helpers, OpenAI-shape chat message and tool request serialization types, a tuned Hyper HTTP/1.1 transport helper, an SSE byte framer, and an OpenAI-compatible stream chunk parser; agent behavior is still planned in `PLAN.md` and has not been implemented yet.
...
- Current implemented internals: CLI parsing, error types, event schema serialization types, event clock timestamps, bounded async JSONL event emission, worktree-rooted path safety helpers, OpenAI chat message/tool request serialization types, a Hyper 1.x + rustls HTTP/1.1 transport helper, an SSE byte framer, and an OpenAI-compatible stream chunk parser.
